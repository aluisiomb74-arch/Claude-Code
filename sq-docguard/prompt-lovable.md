# Prompt para o Lovable — SQ DocGuard
...
- Autenticação: Supabase Auth. Cadastro restrito ao domínio **@sqquimica.com**. O envio de e-mails de cadastro e reset de senha usa o Microsoft Graph (mesmo remetente dos alertas), via fluxo próprio de convites por token — sem SMTP/DNS.
- Armazenamento: Supabase Storage, bucket privado `documentos` (path `{unidade_id}/{documento_id}/{versao}-{arquivo}`).
- Idioma: Português do Brasil. Datas em DD/MM/AAAA. Cor principal da identidade: #ea580c.
...
4. **Alertas de vencimento**: gerados automaticamente em [45, 30, 15, 7, 0] dias antes do vencimento (0 = no dia). Central de notificações in-app (sino + contador). E-mail via Microsoft Graph (resumo diário agrupado por usuário) acionado por cron diário. Intervalos configuráveis pelo admin. Sem envios duplicados.
