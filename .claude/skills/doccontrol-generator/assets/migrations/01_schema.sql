-- ============================================================
-- {{NOME_APP}} — Schema completo (estado consolidado)
...
begin
  ...
  if _dias_list is null then _dias_list := '{{DIAS_ALERTA_JSON}}'::jsonb; end if;
...
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if new.email is null or lower(new.email) not like '%@{{DOMINIO_EMAIL}}' then
    raise exception 'Apenas e-mails do domínio @{{DOMINIO_EMAIL}} são permitidos.';
  end if;
  return new;
end;
$$;
...
begin
  ...
  select id into _unidade_default from public.unidades where nome ilike '{{UNIDADE_PADRAO}}'
